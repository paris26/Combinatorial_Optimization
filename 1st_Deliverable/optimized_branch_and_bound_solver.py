...
class OptimizedNode:
    ...
    def __lt__(self, other):
        ...
        # We want to explore nodes with lower objective values first
...
class OptimizedBranchAndBoundSolver:
    ...
    with a priority queue instead of a stack.
    """
    ...
    def get_node_priority(self, node):
        bound_quality = node.obj_value
        level_factor = node.level  * 0.1

        return bound_quality + level_factor
    ...
    def solve(self) -> Dict:
        ...
        # Note: Using a list as a heap/priority queue with heapq operations
        priority_queue = [root]
        heapq.heapify(priority_queue)
        
        while priority_queue:
            # Get the most promising node (lowest objective value)
